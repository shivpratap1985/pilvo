...
public class TestNgCustomReport implements IReporter {
  ...
  private String reportTitle = "Via.com India Test Report";
  ...
  public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outdir) {
    ...
    String reportTitle  = suites.get(0).getName();
    ...
    try {
      if (m_methodIndex == 0) {
//        MailReport.mail("Successful Execution", DEFAULT_RECIPIENT, reportTitle
//            .substring(0, reportTitle.indexOf(" ")));
      } else {
//        MailReport.mail("Failed Execution", DEFAULT_RECIPIENT,
//            suites.get(0).getName().substring(0, reportTitle.indexOf(" ")));
      }
  ...
  private void generateResult(ITestResult ans, ITestNGMethod method, int resultSetSize) {
    ...
    if (hasParameters) {
      ...
      writer.println("<td  style=\"border:1px solid #009;padding:.25em .5em\">"
					+ Utils.escapeHtml(Utils.toString(parameters[0], null)) + "</td>");
      // }
      writer.println("</tr>");
    }
  ...
  protected void writeReportTitle(String title) {
    writer.print("<center><h1>" + title + " - " + getDateAsString() + " executed by "
        + System.getProperty("user.name") + "</h1></center>");
  }
